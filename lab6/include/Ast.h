...
// 相较lab5的Ast.h，主要每个类增加了genCode()和typeCheck()
...
protected:
    // 运算中发现ture_list中某个指令为true，该节点为true
    // 之所以说某个，是因为有些指令可能不会走到
    // 每个节点的true_list中，指令的goto目标都一样，由回填决定
    std::vector<Instruction *> true_list;
    std::vector<Instruction *> false_list;
    static IRBuilder *builder; // 用于传递继承属性
...
public:
    ...
    virtual void genCode() = 0;   // 产生中间代码

    std::vector<Instruction *> &trueList() { return true_list; }
    std::vector<Instruction *> &falseList() { return false_list; }
};

class ExprNode : public Node // 表达式节点基类，派生出各种表达式节点
{
public:
